...
struct LambdaRankParam : public dmlc::Parameter<LambdaRankParam> {
  ...
  DMLC_DECLARE_PARAMETER(LambdaRankParam) {
    DMLC_DECLARE_FIELD(topk).set_lower_bound(1).set_default(10)
        .describe("ndcg with topk");
...
//################################ YL modify below #################################################
// objective for lambda rank
class LambdaRankObjTopM : public ObjFunction {
 public:
  void Configure(const std::vector<std::pair<std::string, std::string> >& args) override {
    param_.InitAllowUnknown(args);
  }
  void GetGradient(const std::vector<float>& preds,
    ...
    #pragma omp parallel
    {
      ...
      for (bst_omp_uint k = 0; k < ngroup; ++k) {
        ...
        std::sort(lst.begin(), lst.end(), ListEntry::CmpPred); // score large->small
        rec.resize(lst.size());
        for (unsigned i = 0; i < lst.size(); ++i) {
          rec[i] = std::make_pair(lst[i].label, i); // label, rank
        }
        std::sort(rec.begin(), rec.end(), common::CmpFirst); // label large->small
        // enumerate buckets with same label, for each item in the lst, grab another sample randomly
        for (unsigned i = 0; i < std::min(param_.topk,(int) rec.size()); ) { //rec.size();){//
          unsigned j = i + 1;
          while (j < rec.size() && rec[j].first == rec[i].first) ++j;
          // bucket in [i,j), get a sample outside bucket
          unsigned nleft = i, nright = static_cast<unsigned>(rec.size() - j);
          if (nleft + nright != 0) {
            int nsample = param_.num_pairsample;
            while (nsample --) {
              for (unsigned pid = i; pid < j; ++pid) { // for all sample in the bucket 
                // unsigned ridx = nsample;
                unsigned ridx = std::uniform_int_distribution<unsigned>(0, nleft + nright - 1)(rnd);
                if (ridx < nleft) {
                  pairs.push_back(LambdaPair(rec[ridx].second, rec[pid].second));
                } else {
...
// beta version: NDCG lambda rank
class LambdaRankObjYL : public LambdaRankObjTopM {
 protected:
  void GetLambdaWeight(const std::vector<ListEntry> &sorted_list,
                       std::vector<LambdaPair> *io_pairs) override {
    std::vector<LambdaPair> &pairs = *io_pairs;
    ...
    if (IDCG == 0.0) {
      ...
      for (size_t i = 0; i < pairs.size(); ++i) {
        ...
        float pos_label = static_cast<float>(sorted_list[pos_idx].label);
        float neg_label = static_cast<float>(sorted_list[neg_idx].label);
        float original =
            pos_label * pos_loginv + neg_label * neg_loginv;
        float changed  =
            neg_label * pos_loginv + pos_label * neg_loginv;
        float delta = (original - changed) * IDCG;
        if (delta < 0.0f) delta = - delta;
        pairs[i].weight = delta;
      }
  ...
  inline static float CalcYL(const std::vector<float> &labels) {
    double sumdcg = 0.0;
    for (size_t i = 0; i < labels.size(); ++i) {
      // const unsigned rel = static_cast<unsigned>(labels[i]);
      const double rel = labels[i];
      if (rel != 0) {
        sumdcg += rel / std::log2(static_cast<float>(i + 2));
      }
    }
    return static_cast<float>(sumdcg);
  }
...
// ------------------------------ lambda mart -----------------------------
class LambdaMARTObjTopM : public ObjFunction {
 public:
  void Configure(const std::vector<std::pair<std::string, std::string> >& args) override {
    param_.InitAllowUnknown(args);
  }
  void GetGradient(const std::vector<float>& preds,
    ...
    #pragma omp parallel
    {
      ...
      for (bst_omp_uint k = 0; k < ngroup; ++k) {
        ...
        std::sort(lst.begin(), lst.end(), ListEntry::CmpBoth); // predict score large->small
        // enumerate buckets with same label, for each item in the lst, grab another sample randomly
        int t_numpairs = 0;
        for(unsigned i=0;i<std::min((int)lst.size(),param_.topk);i++)
        {
          for(unsigned j=i+1;j<lst.size();j++)
          {
            if(lst[i].label>lst[j].label){
              pairs.push_back(LambdaPair(i, j));
            }
            else{
              pairs.push_back(LambdaPair(j, i));
            }
            t_numpairs++;
          }
        }
        // get lambda weight for the pairs
        this->GetLambdaWeight(lst, &pairs,param_.topk);
        // rescale each gradient and hessian so that the lst have constant weighted
        float scale = 1.0f;///std::min((int)lst.size(),param_.topk);// / param_.num_pairsample;
        // float scale = 1.0f / t_numpairs;
        if (param_.fix_list_weight != 0.0f) {
          scale *= param_.fix_list_weight / (gptr[k + 1] - gptr[k]);
        }
        for (size_t i = 0; i < pairs.size(); ++i) {
          ...
          // accumulate gradient and hessian in both pid, and nid
          gpair[pos.rindex].grad += g * w;
          gpair[pos.rindex].hess += 1.0f * w * h;
          gpair[neg.rindex].grad -= g * w;
          gpair[neg.rindex].hess += 1.0f * w * h;
        }
      }
      // for(int i=0;i<20;i++)
      // {
      //   std::cout<<"gpair_"<<i<<":"<<info.labels[i]<<" "<<gpair[i].grad<<" "<<gpair[i].hess<<std::endl;
      // }
    }
  }
  const char* DefaultEvalMetric(void) const override {
 ...
 protected:
  ...
  struct ListEntry {
    ...
    // comparator by prediction and label
    inline static bool CmpBoth(const ListEntry &a, const ListEntry &b) {
      if(a.pred==b.pred)
        return a.label<b.label;
      return a.pred > b.pred;
    }
...
// beta version: NDCG lambda rank
class LambdaMARTObjYL : public LambdaMARTObjTopM {
 protected:
  void GetLambdaWeight(const std::vector<ListEntry> &sorted_list,
                       std::vector<LambdaPair> *io_pairs,
                       int topk) override {
    ...
    if (IDCG == 0.0) {
      ...
      IDCG = 1.0f / IDCG;
      for (size_t i = 0; i < pairs.size(); ++i) {
        int pos_idx = pairs[i].pos_index;
        int neg_idx = pairs[i].neg_index;       
        if(pos_idx>=topk && neg_idx>=topk){
          pairs[i].weight = 0.0f;
          continue;
        }
        float pos_loginv = 1.0f / std::log2(pos_idx + 2.0f);
        ...
        float changed  =
            neg_label * pos_loginv + pos_label * neg_loginv;
        float delta = (original - changed) * IDCG;
        if(neg_idx>=topk||pos_idx>=topk){
          if(neg_idx>pos_idx){
            delta = (pos_label - neg_label) * pos_loginv*IDCG;
          } else { // pos_idx is a true neg sample
            delta = (pos_label - neg_label) * neg_loginv*IDCG;
          }
        }
        if (delta < 0.0f) delta = - delta;
        pairs[i].weight = delta;
  ...
  inline static float CalcYL(const std::vector<float> &labels, int topk) {
    double sumdcg = 0.0;
    for (int i = 0; i < std::min((int)labels.size(),topk); ++i) {
      // const unsigned rel = static_cast<unsigned>(labels[i]);
      const double rel = labels[i];
      if (rel != 0) {
        sumdcg += rel / std::log2(static_cast<float>(i + 2));
...
// --------------------------------------------------------------- yl modify
XGBOOST_REGISTER_OBJECTIVE(LambdaRankYL, "rank:yl")
.describe("LambdaRank with yl as objective TopM.")
.set_body([]() { return new LambdaRankObjYL(); });
...
XGBOOST_REGISTER_OBJECTIVE(LambdaMARTObjYL, "rank:mart")
.describe("LambdaMART with yl as objective.")
.set_body([]() { return new LambdaMARTObjYL(); });
